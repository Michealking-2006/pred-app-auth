...
function bindPasswordToggles(root = document) {
  const SHOW_ICON = `
    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 16 16" aria-hidden="true">
      <path fill="currentColor" fill-rule="evenodd" d="M8.06 2C3 2 0 8 0 8s3 6 8.06 6C13 14 16 8 16 8s-3-6-7.94-6zM8 12c-2.2 0-4-1.78-4-4c0-2.2 1.8-4 4-4c2.22 0 4 1.8 4 4c0 2.22-1.78 4-4 4zm2-4c0 1.11-.89 2-2 2c-1.11 0-2-.89-2-2c0-1.11.89-2 2-2c1.11 0 2 .89 2 2z"></path>
    </svg>`;

  const HIDE_ICON = `
    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 16 14" aria-hidden="true">
...
function loginPage() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (error) {
      const message = error.message || "Login failed. Check your details and try again.";
...
function signupPage() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const message = "Account created. Check your email to verify your account.";
    showNotice(message, "success");
    notify("success", message);
    window.setTimeout(() => {
      redirectToAuth("login.html");
...
function forgotPage() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const { error } = await SUPABASE.auth.resetPasswordForEmail(
      normalizeEmail(emailField.value),
      {
...
function resetPage() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (error) {
      const message = error.message || "Password update failed. The reset link may be invalid or expired.";
      showNotice(message, "error");
      notify("error", message);
      return;
    }
...
async function signInWithGoogle() {
  ...
  try {
    const { error } = await SUPABASE.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${APP_ORIGIN}/overview`,
      },
    });
...
async function init() {
  ...
  if (!SUPABASE) {
    const message = "Something went wrong. Please refresh the page and try again.";
    showNotice(message, "error");
    notify("error", message);
    return;
  }
